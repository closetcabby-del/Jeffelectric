(()=>{
  ...
  const injectFeatureVideo=()=>{
    ...
    style.textContent=`
      .jeff-feature-video{position:relative;padding:clamp(58px,7vw,94px) clamp(22px,7vw,110px);background:#070807;color:#fff;border-top:1px solid #272922;border-bottom:1px solid #272922;overflow:hidden}
      ...
      .jeff-feature-inner{position:relative;z-index:1;max-width:1180px;margin:0 auto;display:grid;grid-template-columns:minmax(280px,440px) minmax(300px,430px);justify-content:center;align-items:center;gap:clamp(42px,8vw,116px)}
      .jeff-feature-copy{max-width:440px}
      .jeff-feature-kicker{margin:0 0 16px;color:#f2b21a;font-size:10px;font-weight:900;letter-spacing:.2em;text-transform:uppercase}
      .jeff-feature-copy h2{margin:0 0 20px;color:#fff;font-size:clamp(42px,5vw,68px);line-height:.96;letter-spacing:-.045em;font-weight:900}
      .jeff-feature-copy h2 em{display:block;color:#f2b21a;font-style:normal}
      .jeff-feature-copy>p:not(.jeff-feature-kicker){margin:0 0 24px;color:#c2c5bc;font-size:16px;line-height:1.65}
      .jeff-feature-note{display:flex;align-items:center;gap:10px;color:#eef0e8!important;font-size:12px!important;font-weight:800!important;letter-spacing:.02em}
      .jeff-feature-note:before{content:"▶";display:grid;place-items:center;width:30px;height:30px;flex:0 0 30px;border:1px solid #f2b21a;color:#f2b21a;border-radius:50%;font-size:9px}
      ...
      @media(max-width:900px){.jeff-feature-video{padding:54px 22px 70px}.jeff-feature-inner{grid-template-columns:1fr;gap:34px}.jeff-feature-copy{max-width:600px;margin:0 auto;text-align:center}.jeff-feature-copy h2{font-size:clamp(40px,11vw,58px)}.jeff-feature-copy>p:not(.jeff-feature-kicker){font-size:15px}.jeff-feature-note{justify-content:center}.jeff-feature-frame{width:min(88vw,420px);box-shadow:12px 12px 0 #f2b21a}}
      @media(max-width:720px){.jeff-feature-video{padding:38px 18px 92px}.jeff-feature-inner{gap:26px}.jeff-feature-copy h2{font-size:clamp(38px,12vw,52px)}.jeff-feature-copy>p:not(.jeff-feature-kicker){margin-bottom:17px}.jeff-feature-kicker{font-size:8px;margin-bottom:11px}.jeff-feature-frame{width:min(92vw,390px);box-shadow:9px 9px 0 #f2b21a}.jeff-feature-play{width:82px;height:82px}.jeff-feature-video.is-playing~.mobile-bar{transform:translateY(100%);transition:transform .2s ease}}
    ...
    section.innerHTML=`
      <div class="jeff-feature-inner">
        <div class="jeff-feature-copy">
          <p class="jeff-feature-kicker">Featured · Real Jeff Electric work</p>
          <h2 id="jeff-feature-title">See the work.<em>Hear the story.</em></h2>
          <p>This is the full Jeff Electric video, presented in its original vertical format with sound. Press play when you’re ready.</p>
